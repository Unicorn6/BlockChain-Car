...
function view_user_name(uint n) view public returns(string nam){
    nam=user[n].name;
}
function view_user_adr(uint n) view public returns(string nam){
    nam=user[n].adrs;
}
function view_user_totcar(uint n) view public returns(uint nam){
    nam=user[n].carown;
}
function view_user_totlist(uint n) view public returns(uint nam){
    nam=user[n].vehicle.length;
}
...
function validate_user_id(uint s) view public returns(bool){
    if(user[s].flg==1)
     { return true;}
    else
     return false;
